  ...
  return (
    ...
    >
      {value.map((item, index) => (
        <Badge
          key={item}
          className={cn('px-1 rounded-xl flex items-center gap-1', activeIndex === index && 'ring-2 ring-muted-foreground ')}
          //   variant={'secondary'}
        >
...
>(({ className, ...props }, ref) => {
  const { setOpen, inputValue, setInputValue, activeIndex, setActiveIndex, handleSelect } = useMultiSelect();
